...
// Constants for GDScript binding
const int CANVAS_ITEM_LAYER_WORK = static_cast<int>(CanvasItemLayer::WORK);
const int CANVAS_ITEM_LAYER_ITEMS = static_cast<int>(CanvasItemLayer::ITEMS);
const int CANVAS_ITEM_LAYER_STOCKPILES = static_cast<int>(CanvasItemLayer::STOCKPILES);

int FastTileMap::get_layer_z_index(CanvasItemLayer layer) {
    switch(layer) {
        case CanvasItemLayer::STOCKPILES: return Constants::WORLD_SIZE;
        case CanvasItemLayer::WORK: return Constants::WORLD_SIZE + 1;
        case CanvasItemLayer::ITEMS: return 0;
        default: return Constants::WORLD_SIZE + 1;
    }
}
...
void FastTileMap::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("add_canvas_item", "layer", "cellPos", "texture", "atlas"), &FastTileMap::add_canvas_item);
    ClassDB::bind_method(D_METHOD("remove_canvas_item", "layer", "cellPos"), &FastTileMap::remove_canvas_item);
    ClassDB::bind_method(D_METHOD("clear_canvas_layer", "layer"), &FastTileMap::clear_canvas_layer);
    
    // Bind enum
    BIND_ENUM_CONSTANT(CANVAS_ITEM_LAYER_WORK);
    BIND_ENUM_CONSTANT(CANVAS_ITEM_LAYER_ITEMS);
    BIND_ENUM_CONSTANT(CANVAS_ITEM_LAYER_STOCKPILES);
}
...
FastTileMap::~FastTileMap() {
    ...
    for (auto& layer_pair : canvas_item_layers) {
        for (auto& item_pair : layer_pair.second) {
            if (item_pair.second.is_valid()) {
                RenderingServer::get_singleton()->free_rid(item_pair.second);
            }
        }
    }
    canvas_item_layers.clear();
}
...
void FastTileMap::add_canvas_item(int layer, Vector2i cellPos, Ref<Texture2D> texture, Vector2i atlas) {
    CanvasItemLayer enum_layer = static_cast<CanvasItemLayer>(layer);
    remove_canvas_item(layer, cellPos);
    ...
    RenderingServer::get_singleton()->canvas_item_set_z_index(canvas_item, get_layer_z_index(enum_layer));
    ...
    canvas_item_layers[enum_layer][cellPos] = canvas_item;
}

void FastTileMap::remove_canvas_item(int layer, Vector2i cellPos) {
    CanvasItemLayer enum_layer = static_cast<CanvasItemLayer>(layer);
    auto layer_it = canvas_item_layers.find(enum_layer);
    if (layer_it == canvas_item_layers.end()) return;
    
    auto item_it = layer_it->second.find(cellPos);
    if (item_it != layer_it->second.end()) {
        RenderingServer::get_singleton()->free_rid(item_it->second);
        layer_it->second.erase(item_it);
    }
}

void FastTileMap::clear_canvas_layer(int layer) {
    CanvasItemLayer enum_layer = static_cast<CanvasItemLayer>(layer);
    auto layer_it = canvas_item_layers.find(enum_layer);
    if (layer_it == canvas_item_layers.end()) return;
    
    for (auto& pair : layer_it->second) {
        RenderingServer::get_singleton()->free_rid(pair.second);
    }
    layer_it->second.clear();
}